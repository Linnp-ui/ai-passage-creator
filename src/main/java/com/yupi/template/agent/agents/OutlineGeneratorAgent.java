...
public class OutlineGeneratorAgent implements NodeAction {
    ...
    public Map<String, Object> apply(OverAllState state) throws Exception {
        ...
        // 解析结果
        ArticleState.OutlineResult outlineResult = GsonUtils.fromJsonSafe(
                content, 
                ArticleState.OutlineResult.class
        );
        
        // 处理解析失败的情况 - 自动重新生成
        if (outlineResult == null || outlineResult.getSections() == null) {
            log.warn("OutlineGeneratorAgent JSON解析失败，尝试重新生成");
            
            // 重新调用LLM生成大纲
            String retryContent = callLlmWithStreaming(prompt, streamHandler);
            outlineResult = GsonUtils.fromJsonSafe(
                    retryContent, 
                    ArticleState.OutlineResult.class
            );
            ...
            // 如果重新生成仍然失败，使用默认大纲结构
            if (outlineResult == null || outlineResult.getSections() == null) {
                log.error("OutlineGeneratorAgent 重新生成仍失败，使用默认大纲结构");
                outlineResult = new ArticleState.OutlineResult();
                ArticleState.OutlineSection section = new ArticleState.OutlineSection();
                section.setSection(1);
                section.setTitle("内容概述");
                section.setPoints(List.of("主要内容介绍", "核心观点阐述", "总结与展望"));
                outlineResult.setSections(List.of(section));
            } else {
                log.info("OutlineGeneratorAgent 重新生成成功");
            }
        }
