...
public class ArticleController {
     ...
     * 检查文章是否可以续生成
     */
    @GetMapping("/resume/check/{taskId}")
    @Operation(summary = "检查文章是否可以续生成")
    public BaseResponse<ArticleResumeService.ResumeInfo> checkResumeStatus(
            @PathVariable String taskId, HttpServletRequest httpServletRequest) {
        ThrowUtils.throwIf(taskId == null || taskId.trim().isEmpty(),
                ErrorCode.PARAMS_ERROR, "任务ID不能为空");

        User loginUser = userService.getLoginUser(httpServletRequest);
        ArticleResumeService.ResumeInfo info = articleResumeService.getResumeInfo(taskId, loginUser);
    ...
    @Operation(summary = "继续生成文章")
    public BaseResponse<Boolean> resumeArticle(@PathVariable String taskId, HttpServletRequest httpServletRequest) {
        ThrowUtils.throwIf(taskId == null || taskId.trim().isEmpty(),
                ErrorCode.PARAMS_ERROR, "任务ID不能为空");

        User loginUser = userService.getLoginUser(httpServletRequest);

        boolean result = articleResumeService.resumeArticle(taskId, loginUser);
