...
public interface PromptConstant {
    ...
    String AGENT2_OUTLINE_PROMPT = """
            ...
            示例输出 (仅供参考):
            {
              "sections": [
                {
                  "section": 1,
                  "title": "引言：为什么AI创作如此重要",
                  "points": ["AI创作的现状和发展趋势", "AI创作对内容行业的影响"]
                },
                {
                  "section": 2,
                  "title": "AI创作的核心技术",
                  "points": ["自然语言处理技术", "深度学习模型"]
                },
                {
                  "section": 3,
                  "title": "AI创作的应用场景",
    ...
    /**
     * 图库搜索关键词优化 Prompt
     * 将中文/模糊关键词翻译并优化为适合图库检索的英文关键词
     */
    String IMAGE_KEYWORDS_OPTIMIZATION_PROMPT = """
            你是一位专业的图片搜索优化专家，擅长将用户需求转化为高质量的图库搜索关键词。
            
            请将以下图片描述优化为适合图库搜索的英文关键词：
            原始描述：{originalKeywords}
            
            要求：
            1. 将中文翻译为准确、具体的英文关键词
            2. 使用图库友好的通用词汇（避免过于抽象或生僻的词）
            3. 生成 3 组不同角度的关键词变体，提高搜索命中率
            4. 每组关键词控制在 2-4 个词，用空格分隔
            5. 优先使用摄影图库中常见的标签词汇
