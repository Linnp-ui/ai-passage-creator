...
public class ImageServiceStrategy {
    ...
    public ImageResult getImageAndUpload(String imageSource, ImageRequest request) {
        ...
        try {
            // 对于 AI 生图服务，使用 getImageData 获取字节数据并保存到本地
            if (method.isAiGenerated()) {
                ...
                if (imageData != null && imageData.getBytes() != null) {
                    ...
                    String imageUrl = localImageStorageService.saveImage(
                            imageData.getBytes(),
                            imageData.getMimeType(),
                            folder
                    );
                    if (imageUrl != null) {
                        log.info("AI 图片生成成功, method={}, url={}", method, imageUrl);
                        return new ImageResult(imageUrl, method);
                    }
                }
                log.warn("AI 图片生成失败, 使用降级方案, method={}", method);
                return handleFallback(request.getPosition());
            } else {
                // 对于图库检索服务，使用 getImage 获取 URL
                String imageUrl = service.getImage(request);
                if (imageUrl != null && !imageUrl.isEmpty()) {
                    log.info("图片获取成功, method={}, url={}", method, imageUrl);
                    return new ImageResult(imageUrl, method);
                } else {
            ...
            log.error("获取图片异常, method={}", method, e);
            return handleFallback(request.getPosition());
        }
    }
