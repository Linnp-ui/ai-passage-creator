...
public class PaymentServiceImpl implements PaymentService {
    ...
    public String createVipPaymentSession(Long userId) throws StripeException {
        // Ensure Stripe configuration is provided
        if (stripeConfig.getApiKey() == null || stripeConfig.getApiKey().isEmpty() ||
                stripeConfig.getSuccessUrl() == null || stripeConfig.getSuccessUrl().isEmpty() ||
                stripeConfig.getCancelUrl() == null || stripeConfig.getCancelUrl().isEmpty()) {
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "Stripe 配置缺失，无法创建 VIP 支付会话");
        }
