...
public class ArticleAsyncService {
    ...
    public void executePhase1(String taskId, String topic, String style) {
        ...
        try {
            ...
            // 推送阶段切换消息
            sseEmitterManager.sendMessage(taskId, SseMessageTypeEnum.PHASE_CHANGED, 
                Map.of("phase", ArticlePhaseEnum.TITLE_GENERATING.getValue(), "message", "开始生成标题"));
            ...
            // 保存状态快照
            articleStateManager.saveSnapshot(taskId, state, ArticleGenerationCheckpoint.TITLE_OPTIONS_GENERATED);
    ...
    public void executePhase2(String taskId) {
        ...
        try {
            ...
            // 推送阶段切换消息
            sseEmitterManager.sendMessage(taskId, SseMessageTypeEnum.PHASE_CHANGED, 
                Map.of("phase", ArticlePhaseEnum.OUTLINE_GENERATING.getValue(), "message", "开始生成大纲"));
            ...
            // 保存状态快照
            articleStateManager.saveSnapshot(taskId, state, ArticleGenerationCheckpoint.OUTLINE_GENERATED);
    ...
    public void executePhase3(String taskId) {
        ...
        try {
            ...
            // 推送阶段切换消息
            sseEmitterManager.sendMessage(taskId, SseMessageTypeEnum.PHASE_CHANGED, 
                Map.of("phase", ArticlePhaseEnum.CONTENT_GENERATING.getValue(), "message", "开始生成正文"));
            ...
            // 设置大纲
            List<ArticleState.OutlineSection> outlineSections = Collections.emptyList();
            if (article.getOutline() != null && !article.getOutline().isEmpty()) {
                outlineSections = GsonUtils.fromJson(
                        article.getOutline(),
                        new TypeToken<List<ArticleState.OutlineSection>>(){}
                );
                if (outlineSections == null) {
                    outlineSections = Collections.emptyList();
                }
            }
            ...
            if (useOrchestrator) {
                articleAgentOrchestrator.executePhase3_GenerateContent(state, message -> {
                    handleAgentMessage(taskId, message, state);
                    // 在正文生成完成后保存状态
                    if (message.contains("AGENT3_COMPLETE")) {
                        articleStateManager.saveSnapshot(taskId, state,
                            ArticleGenerationCheckpoint.CONTENT_GENERATED, 60, "ContentGeneratorAgent");
                    }
                    // 在配图需求分析完成后保存状态
                    if (message.contains("AGENT4_COMPLETE")) {
                        articleStateManager.saveSnapshot(taskId, state,
                            ArticleGenerationCheckpoint.IMAGE_REQUIREMENTS_ANALYZED, 65, "ImageAnalyzerAgent");
                    }
                    // 在每张配图完成后保存状态
                    if (message.contains("IMAGE_COMPLETE")) {
                        int progress = 70 + (state.getImages() != null ? state.getImages().size() * 5 : 0);
                        progress = Math.min(progress, 90);
                        articleStateManager.saveSnapshot(taskId, state,
                            ArticleGenerationCheckpoint.IMAGE_PARTIAL_COMPLETE, progress, "ImageGeneratorAgent");
                    }
                });
            } else {
            ...
            // 保存最终状态快照
            articleStateManager.saveSnapshot(taskId, state, ArticleGenerationCheckpoint.GENERATION_COMPLETE);
