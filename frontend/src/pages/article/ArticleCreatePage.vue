            ...
            <div class="input-area">
              ...
              <div class="image-methods-section">
                ...
                <a-checkbox-group v-model:value="selectedImageMethods" class="methods-group">
                  ...
                  <a-checkbox value="UNSPLASH">Unsplash</a-checkbox>
                  <a-checkbox value="AGGREGATED">智能聚合</a-checkbox>
                  <a-tooltip :title="isVip ? '' : '仅限 VIP 会员'">
                    <a-checkbox value="DASHSCOPE" :disabled="!isVip">
                      通义万相
                  ...
                  <a-tooltip :title="isVip ? '' : '仅限 VIP 会员'">
                    <a-checkbox value="HUGGINGFACE" :disabled="!isVip">
                      Hugging Face
                      <CrownOutlined v-if="!isVip" class="vip-icon" />
                    </a-checkbox>
                  </a-tooltip>
    ...
    >
      <div class="resume-dialog-content">
        ...
        <div class="resume-info">
          <h3>是否继续上次创作？</h3>
          <p class="resume-desc">检测到您有未完成的创作任务</p>
          <div class="resume-details">
            <div class="resume-item">
              <span class="label">当前阶段：</span>
              <span class="value">{{ resumeInfo?.phaseDescription }}</span>
            </div>
            <div class="resume-item">
              <span class="label">生成进度：</span>
              <a-progress :percent="resumeInfo?.progressPercent || 0" size="small" />
            </div>
            <div class="resume-item">
              <span class="label">最后状态：</span>
              <span class="value">{{ resumeInfo?.message }}</span>
            </div>
          </div>
        </div>
        <div class="resume-actions">
          <a-button @click="handleCancelResume" :disabled="isResuming">
            重新开始
          </a-button>
          <a-button type="primary" @click="handleConfirmResume" :loading="isResuming">
            继续创作
          </a-button>
        </div>
      </div>
...
import { createArticle, confirmTitle, confirmOutline, checkResumeStatus, resumeArticle } from '@/api/articleController'
...
// 配额相关计算属性
const isAdmin = computed(() => checkIsAdmin(loginUserStore.loginUser ?? undefined))
const isVip = computed(() => checkIsVip(loginUserStore.loginUser ?? undefined))
const quota = computed(() => loginUserStore.loginUser?.quota ?? 0)
const hasQuota = computed(() => checkHasQuota(loginUserStore.loginUser ?? undefined))
...
const resumeInfo = ref<API.ResumeInfo | null>(null)
...
const handleSSEMessage = (msg: SSEMessage) => {
  ...
  switch (msg.type) {
    ...
    case 'TITLES_GENERATED':
      ...
      isCreating.value = false
      addLog(`生成了 ${(msg.titleOptions as Array<unknown>)?.length || 0} 个标题方案`, 'success')
      break
    ...
    case 'AGENT4_COMPLETE':
      // 配图分析完成，进入配图生成步骤
      currentStep.value = 4
      totalImages.value = (msg.imageRequirements as Array<unknown>)?.length || 5
    ...
    case 'AGENT5_COMPLETE':
      // 所有配图完成，进入图文合成步骤
      currentStep.value = 5
      article.value.images = (msg.images as API.ImageItem[]) || []
      addLog('所有配图生成完成', 'success')
      break

    case 'MERGE_COMPLETE':
      // 图文合成完成
      article.value.fullContent = (msg.fullContent as string) || ''
      scrollToBottom()
    ...
    case 'ERROR':
      errorMessage.value = (msg.message as string) || '创作失败'
      errorVisible.value = true
      isCreating.value = false
      currentPhase.value = 'INPUT'
      addLog(`创作失败: ${(msg.message as string) || '未知错误'}`, 'error')
      break
  }
}
...
onMounted(() => {
  ...
  // 检查是否有任务ID，如果有则检查续生成状态
  if (route.query.taskId) {
    checkResumeState(route.query.taskId as string)
  }
})

// 检查续生成状态
const checkResumeState = async (taskId: string) => {
  try {
    const res = await checkResumeStatus({ taskId })
    if (res.data.data && res.data.data.canResume) {
      resumeInfo.value = res.data.data
      showResumeDialog.value = true
    }
  } catch (error) {
    console.error('检查续生成状态失败', error)
  }
}
...
const handleConfirmResume = async () => {
  ...
  try {
    const res = await resumeArticle({ taskId: resumeInfo.value.taskId })
    if (res.data.data) {
      message.success('已启动续生成')
      showResumeDialog.value = false
      // 设置当前任务ID并连接SSE
      taskId.value = resumeInfo.value.taskId
      currentPhase.value = 'TITLE_GENERATING'
      isCreating.value = true
      // 建立 SSE 连接
      eventSource = connectSSE(taskId.value, {
    ...
    const err = error as Error
    message.error(err.message || '续生成失败')
  } finally {
