    ...
    <div class="stage-header">
      ...
      <div class="toolbar">
        ...
        <div class="toolbar-right">
          <span v-if="autoSaveStatus === 'saving'" class="save-status saving">
            <SaveOutlined spin /> 保存中...
          </span>
          <span v-else-if="autoSaveStatus === 'saved'" class="save-status saved">
            <CheckOutlined /> 已自动保存
          </span>
        </div>
      </div>
...
import { CheckOutlined, DeleteOutlined, PlusOutlined, RobotOutlined, CrownOutlined, UndoOutlined, RedoOutlined, SaveOutlined } from '@ant-design/icons-vue'
...
const historyStack = ref<OutlineSection[][]>([])
...
const saveToHistory = () => {
  ...
  const currentState = JSON.parse(JSON.stringify(outlineSections.value))
...
const undo = () => {
  ...
  historyIndex.value--
  outlineSections.value = JSON.parse(JSON.stringify(historyStack.value[historyIndex.value]))
  nextTick(() => {
...
const getStorageKey = () => `outline_draft_${props.taskId}`

const saveToLocalStorage = () => {
  try {
    localStorage.setItem(getStorageKey(), JSON.stringify(outlineSections.value))
    localStorage.setItem(`${getStorageKey()}_time`, new Date().toISOString())
...
const loadFromLocalStorage = (): OutlineSection[] | null => {
  try {
...
const clearLocalStorage = () => {
  try {
    ...
    localStorage.removeItem(`${getStorageKey()}_time`)
  } catch (e) {
...
onMounted(() => {
  ...
  if (savedDraft && savedDraft.length > 0) {
    const draftTime = localStorage.getItem(`${getStorageKey()}_time`)
    if (draftTime) {
      const confirmRestore = window.confirm(
        `检测到 ${new Date(draftTime).toLocaleString()} 的未保存草稿，是否恢复？`
      )
      if (confirmRestore) {
        outlineSections.value = savedDraft
